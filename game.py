...
class Game:
    ...
    def __init__(self, player_1, player_2):
        ...
        self.win_history = []
        self.win_mapping = {1: f"{self.player_dict[1].name}'s win", 2: f"{self.player_dict[2].name}'s win", 999: 'Draw'}
    ...
    def checkOneRoundResult(self):
        if self.player_used_points_dict[self.first_player][-1] > self.player_used_points_dict[self.second_player][-1]:
            win_number = self.first_player
        elif self.player_used_points_dict[self.first_player][-1] < self.player_used_points_dict[self.second_player][-1]:
    ...
    def checkGameResult(self):
        if (self.round == 9) or (self.win_history.count(1) == 5) or (self.win_history.count(2) == 5):
            self.endGame()
    ...
    def endGame(self):
        ...
        for i in range(self.round):
            ...
            print(f"{self.player_dict[self.first_player_history[i]].name}: {self.player_used_points_dict[self.first_player_history[i]][i]}")
            print(f"{self.player_dict[self.second_player_history[i]].name}: {self.player_used_points_dict[self.second_player_history[i]][i]}")
        exit()
